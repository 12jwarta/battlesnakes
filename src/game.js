...
function createStartingFood(width, height, snakes, rng = Math.random) {
...
export function createGameState(options = {}) {
  ...
  const startingPlayerSize = Math.max(1, Math.floor(options.startingPlayerSize ?? 1));
  const startingEnemySize = Math.max(1, Math.floor(options.startingEnemySize ?? 1));
  ...
  let enemySnake = buildInitialSnake(
    { x: enemyStartX, y: startY },
    "LEFT",
    startingEnemySize,
    width,
    height
  ).map(cloneCell);
  const startingFood = createStartingFood(width, height, [playerSnake, enemySnake], options.rng);
...
function chooseEnemyDirection(state, rng = Math.random) {
  ...
  if (!state.food.point && isDirectionSafeNow(state, currentDirection, currentDirection)) {
    return currentDirection;
  }
...
export function advanceGame(state, rng = Math.random) {
  ...
  const playerTookDamage = playerHitBoundary || playerSelfHit || playerHitEnemyBody || headOnHeadHit;
  const enemyTookDamage = enemyHitBoundary || enemySelfHit || enemyHitPlayerBody || headOnHeadHit;
  ...
  for (const playerCandidate of playerCandidates) {
    for (const enemyCandidate of enemyCandidates) {
      const playerValid = isSnakePlacementValid(
        playerCandidate,
        state.width,
        state.height,
        enemyCandidate
      );
      const enemyValid = isSnakePlacementValid(
        enemyCandidate,
        state.width,
        state.height,
        playerCandidate
  ...
  let playerDelta = playerTookDamage ? -1 : 0;
  let enemyDelta = enemyTookDamage ? -1 : 0;
  ...
  const playerDroppedBelowLength = playerTookDamage && state.player.snake.length <= 1;
  const enemyDroppedBelowLength = enemyTookDamage && state.enemy.snake.length <= 1;
  ...
  const nextFood = shouldRespawnFoodSet
  ...
  const finalStatus = !nextFood.point && finalSpawnTimer === 0 && finalRespawnSide === null && status === "running"
    ? "draw"
    : status;
